...
class CategoryModel extends Model {
	...
	function get_category_option($root_id = 0, $cate_id = 0, $level_id = 0) {
		...
		foreach ( $categories as $cate ) {
			...
			if ($cate_id > 0 && $cate_id == $cate ['cate_id'])
				$optstr .= ' selected';
			if ($level_id == 0) {
	 ...
	 * }
	 */
	function add_postcount($cate_id = 0) {
		$row = $this->get_one_category ( $cate_id );
		$data ['cate_postcount'] = $row ['cate_postcount'] + 1;
		$this->Category->where ( 'cate_id=' . $cate_id )->save ( $data );
		$cate_id = null;
		$row = null;
		$data = null;
	}
	function del_postcount($cate_id = 0) {
		$row = $this->get_one_category ( $cate_id );
		$data ['cate_postcount'] = $row ['cate_postcount'] - 1;
		$this->Category->where ( 'cate_id=' . $cate_id )->save ( $data );
		$cate_id = null;
